...
/*   Updated: 2020/01/15 19:06:49 by jchotel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		display_lifebar(t_game *game)
{
	...
	display_rec(game, form(vec(i->width / 4 - 5, i->height - 40),
			vec(i->width / 2 + 10, 30), 0xFFFBBC), &i);
	display_rec(game, form(vec(i->width / 4, i->height - 35),
			size, color), &i);
	size = vec((i->width / 4) * (game->p.rot_speed / (float)MAX_ROT_SPEED), 5);
	display_rec(game, form(vec((i->width / 4), i->height - 20), size, 0xD56DFF), &i);
	size = vec((i->width / 4) * game->p.speed, 5);
	display_rec(game, form(vec((3 * i->width / 4) - size.x, i->height - 20), size, 0x0000FF), &i);
	return (1);
}
	...
	while (game->map->map[y])
	{
		...
		while (game->map->map[y][x])
		{
			...
			if (ft_strchr("2LCU", game->map->map[y][x]))
				display_cir2(game, form(vec(game->dim.x / MAP_SIZE * (x + 0.5f), game->dim.x / MAP_SIZE * (y + 0.5f)), vec(0,5), 0xFF2B12));
			x++;
		}
		y++;
	}
...
{
	display_rec(game, form(vec(game->dim.x - 100, 5), vec(100, 30), 0x00FFFF), &game->image),
		...
		if (!game->disable_map)
		{
			...
			display_tri(game, form(vec((game->dim.x / MAP_SIZE) * game->p.pos.x,
						...
						vec(game->dim.x / MAP_SIZE + 3, game->dim.x / MAP_SIZE + 3),
								PLAYER_COLOR));
			display_cir2(game, form(vec((game->dim.x / MAP_SIZE) * game->p.pos.x,
								(game->dim.x / MAP_SIZE) * game->p.pos.y),
