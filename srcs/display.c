...
/*   Updated: 2019/12/17 12:55:17 by llaurent         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_vector next_inter(t_vector *p, t_vector vec, float teta, int *wall, t_game *game)
{
	...
	x.x = (int) vec.x + (cos(alpha) > 0 ? 1 : -1);
	x.y = -tan(alpha) * (float) x.x + c;
	y.y = (int) vec.y + (sin(alpha) > 0 ? -1 : 1);
	y.x = (y.y - c) / -tan(alpha);
	res = (sq_dist(p, &y) > sq_dist(p, &x) ? x : y); //ligne a mettre dans le return
	//printf("next x : %f  next y :  %f wall : %d\n",next.x, next.y, wall);
	mlx_pixel_put(game->ptr, game->win, x.x*game->map->tex.size, x.y*game->map->tex.size, 0xFFFFFF);
	mlx_pixel_put(game->ptr, game->win, y.x*game->map->tex.size, y.y*game->map->tex.size, 0xFFFFFF);
...
t_vector next_hit(t_map *map, t_vector *p, float teta, int *wall, t_game *game)
{
	...
	while (map->map[(int)(res.y)][(int)(res.x)] != '1' && map->map[(int)(res.y-0.01)][(int)(res.x-0.01)] != '1' && map->map[(int)(res.y+0.01)][(int)(res.x+0.01)] != '1' && map->map[(int)(res.y-0.01)][(int)(res.x)] != '1' && map->map[(int)(res.y)][(int)(res.x-0.01)] != '1' && map->map[(int)(res.y)][(int)(res.x+0.01)] != '1' && map->map[(int)(res.y+0.01)][(int)(res.x)] != '1')
	{
...
int display_full_range(t_game *game)
{
	...
	if (game->save_first_image)
	{
		...
		ft_putstr("Screenshot saved at \"screenshot.bmp\"\n");
	}
	return (1);
